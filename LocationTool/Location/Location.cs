    ...
    namespace LocationTool
    {
        ...
        public class LocationBase
        {
            ...
            public List<Vector3> GetUniformPointsDistribution(int spacing = 1)
            {
                ...
                { Debug.LogError("invalid location !"); return uniformPoints; }
                ...
                for (int i = 0; i < s; i++)
                {
                    ...
                    if ((minBounds + point).x > maxBounds.x)
                        break;
                    ...
                    for (int j = 0; j < s; j++)
                    {
                        ...
                        /*
                        if (mcSystems.GeometricUtilities_2d.PointInsidePolygon(
                            Boundaries2d,
                            Boundaries2d.Length,
                            new Vector2(minBounds.x + point.x, minBounds.z + point.z)))
                            uniformPoints.Add(minBounds + point);

                        if ((minBounds + point).z > maxBounds.z)
                            break;
                            */
                    }
                }
            ...
            public Vector2 GetCenter()
            {
                ...
                Vector2[] positions = Boundaries2d;
                Vector2 centroid = Utils.GeometricUtilities_2d.Compute2DPolygonCentroid(positions,
                    positions.Length);
        ...
        public class Location : MonoBehaviour
        {
            ...
            AutoEditor.AutoEditor locationAutoEd = null;
            public AutoEditor.AutoEditor AutoEditor
            {
                get
                {
                    if (locationAutoEd == null)
                    {
                        ...
                        locationAutoEd = new AutoEditor.AutoEditor(typeof(LocationBase), ref obj);
                    }
            ...
            public void Awake()
            {
                GameObject.FindGameObjectWithTag("LT_Globals").GetComponent<LT_Globals>().Locations.Add(locationBase);
            }
